...
export default function LevelSelect({ onSelectLevel, progress, onBack }: LevelSelectProps) {
  return (
    <div className="flex-1 flex flex-col px-6 pt-6 pb-24">
      ...
      {/* Current game mode — more modes coming soon */}
      <motion.div
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
      ...
      >
        <div className="bg-white/60 rounded-full px-4 py-2 flex items-center gap-2 text-sm font-display font-semibold text-emerald-600">
          <span className="text-lg">🌱</span> Super Sprout
        </div>
      </motion.div>
    </div>
  );
